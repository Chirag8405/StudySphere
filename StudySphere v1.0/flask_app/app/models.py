...
def create_assignment(user_id, name, due_date):
    ...
    try:
        with connection.cursor() as cursor:
            cursor.execute(
                "INSERT INTO Assignments (user_id, name, due_date, status) VALUES (%s, %s, %s, %s)",
                (user_id, name, due_date, 'pending')  # Default status set to 'pending'
            )
            connection.commit()
...
def get_assignments(user_id):
    ...
    try:
        with connection.cursor(dictionary=True) as cursor:
            ...
            result_assignments = cursor.fetchall()
