...
def update_attendance(lecture_id, status):
    connection = create_connection()
    cursor = connection.cursor()
    cursor.execute("UPDATE Attendance SET status = %s WHERE id = %s", (status, lecture_id))
    connection.commit()
    cursor.close()
    connection.close()
        ...
        with connection.cursor(dictionary=True) as cursor:
            ...
            for entry in records:
                ...
                if isinstance(entry['start_time'], datetime):
                    entry['start_time'] = entry['start_time'].isoformat()  # Convert to ISO format
                    
                if isinstance(entry['end_time'], datetime):
                    entry['end_time'] = entry['end_time'].isoformat()  # Convert to ISO format 
...
def update_timetable_entry(entry_id, subject, start_time, end_time, days):
    ...
    try:
        with connection.cursor() as cursor:
            cursor.execute("UPDATE Timetable SET subject = %s, start_time = %s, end_time = %s, days = %s WHERE id = %s", (subject, start_time, end_time, ','.join(days), entry_id))  # Join days into a string
            connection.commit()
...
def get_user_profile(user_id):
    ...
    try:
        ...
        user = cursor.fetchone()
        return user  # Return user profile data
    except Exception as e:
...
def get_assignments_summary(user_id):
    ...
    try:
        cursor = connection.cursor(dictionary=True)
        cursor.execute("SELECT COUNT(*) AS upcoming FROM assignments WHERE user_id = %s AND due_date > NOW()", (user_id,))
        upcoming = cursor.fetchone()['upcoming']
        
        cursor.execute("SELECT COUNT(*) AS overdue FROM assignments WHERE user_id = %s AND due_date < NOW()", (user_id,))
        overdue = cursor.fetchone()['overdue']
        
        return {"upcoming": upcoming, "overdue": overdue}  # Return summary data
    except Exception as e:
...
def get_attendance_summary(user_id):
    ...
    try:
        ...
        return {
            "attended": attended,
            "missed": missed,
            "percentage": round(percentage, 2)  # Return attendance summary data
        }
    except Exception as e:
...
def get_user_interests(user_id):
    ...
    try:
        ...
        interests = [row['interest'] for row in cursor.fetchall()]  # Fetch all interests
        
        return interests  # Return list of interests
    except Exception as e:
