...
def mark_attendance(lecture_id):
    ...
    update_attendance(lecture_id, status)
...
def edit_timetable_entry(entry_id):
    ...
    if entry:
        return jsonify(entry)  # Return the entry as JSON
    else:
...
def update_entry(entry_id):
    ...
    start_time = data.get('start_time')
    end_time = data.get('end_time')
    days = data.get('days')  # Expecting days as a list
...
def get_attendance_timetable():
    user_id = request.args.get('user_id')
    today = datetime.now().strftime('%A')  # Get current day as a string (e.g., 'Monday')
    try:
        connection = create_connection()
        cursor = connection.cursor(dictionary=True)
        cursor.execute("SELECT * FROM Timetable WHERE FIND_IN_SET(%s, days)", (today,))
        entries = cursor.fetchall()  # Fetch all results
        
        # Convert timedelta to string if necessary
        for entry in entries:
            if isinstance(entry['start_time'], timedelta):  # Assuming 'duration' is a field that may contain timedelta
                entry['start_time'] = str(entry['start_time'])  # Convert to string or format as needed
                
            if isinstance(entry['end_time'], timedelta):  # Assuming 'duration' is a field that may contain timedelta
                entry['end_time'] = str(entry['end_time'])  # Convert to string or format as needed
...
# Save extra lecture (for demonstration purposes)
