document.addEventListener("DOMContentLoaded", function () {
  ...
  extraLectureForm.addEventListener("submit", function (event) {
    ...
    const days = Array.from(
      document.getElementById("extra-days").selectedOptions
  ...
  // Function to display attendance records for today only
  function displayAttendance(attendanceRecords) {
    ...
    attendanceRecords.forEach((entry) => {
      ...
      if (entry.days.includes(currentDay)) {
        ...
        percentageElement.textContent = `Attendance %: ${calculateIndividualAttendance(
          entry
        )}%`;
        ...
        // Append all elements to entry element
        ...
        // Append buttons to entry element
        entryElement.appendChild(markPresentBtn);
        entryElement.appendChild(markAbsentBtn);
        entryElement.appendChild(markCancelledBtn);

        // Log to check if buttons are created
        console.log("Created buttons for:", entry.subject);

        // Append the entire entry element to the attendance list
  ...
  function fetchTimetable() {
    fetch("/api/attendance_timetable")
      .then((response) => {
        if (!response.ok) {
          throw new Error("Network response was not ok");
        }
  ...
  function displayTimetable(timetableData) {
    ...
    timetableData.forEach((entry) => {
      ...
      const timeElement = document.createElement("p");
      timeElement.textContent = `${new_start_time} - ${new_end_time}`;

      const daysElement = document.createElement("p");
      daysElement.textContent = `${entry.days}`;
      ...
      attendanceDiv.appendChild(subjectElement);
      attendanceDiv.appendChild(timeElement);
      attendanceDiv.appendChild(daysElement);
      ...
      entryElement.appendChild(attendanceDiv);
